...
const options = ["Feminino", "Outros", "Prefiro não dizer"];
...
export const GenderScreen: React.FC<GenderScreenProps> = ({ onNext, onBack, progress }) => {
    ...
    return (
        <OnboardingLayout 
            title="Qual opção representa melhor você?" 
            showSkip onSkip={onNext} 
            showBack onBack={onBack}
            progress={progress}
        >
            <div role="radiogroup" aria-label="Opções de gênero" className="space-y-3 w-full">
                {options.map(option => (
                     <button
                        key={option}
