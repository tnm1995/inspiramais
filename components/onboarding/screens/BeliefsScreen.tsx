...
const options = ["Cristianismo", "Judaísmo", "Islamismo", "Hinduísmo", "Budismo", "Outro"];
...
export const BeliefsScreen: React.FC<BeliefsScreenProps> = ({ onNext, onBack, progress }) => {
    const [selected, setSelected] = useState<string | null>(null);
    const { updateUserData } = useUserData();

    const handleSelect = (option: string) => {
        setSelected(option);
        updateUserData({ beliefs: option });
        setTimeout(() => {
            onNext();
        }, 300);
    };
    ...
    return (
        <OnboardingLayout 
            title="Qual descreve melhor suas crenças?" 
            showSkip onSkip={onNext} 
            showBack onBack={onBack}
            progress={progress}
        >
            <div role="radiogroup" aria-label="Opções de crença espiritual" className="space-y-3 w-full">
                {options.map(option => (
                    <button
                        key={option}
